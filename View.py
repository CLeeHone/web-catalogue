...
# Sometimes, an error related to favicon.ico is thrown, especially after a hard reload. Oddly, removing this function,
# and running it again, stops that exception. The exception itself does not break the program.
@app.route("/<string:item_id>")
def view_item(item_id):
	item = coordinator.get_item_data(item_id)
	reviews = list(coordinator.get_item_reviews(item_id))
	average_stars = coordinator.get_average_rating(reviews)
	return render_template("itemDetails.html", item=item, reviews=reviews, average_stars=average_stars)
