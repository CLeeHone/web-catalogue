...
class BusinessLogic:
    ...
    def insert_item_review(self, item_id):
        self.db_communicator.insert_item_review(self, item_id)
    ...
    def get_average_rating(reviews):
        ...
        for review in reviews:
            if review["stars"]:
                review_count += 1
                star_sum += int(review["stars"])

        # We actually want integer division here, as we can't have half stars
        return int(star_sum/review_count) if review_count > 0 else 0
